...
const projects = [
  {
    ...
    description: 'Built with React, MUI, and Framer Motion.',
    image: 'https://via.placeholder.com/300x200',
    link: '#'
  },
  {
    title: 'Weather App',
    description: 'Displays weather using OpenWeatherMap API.',
    image: 'https://via.placeholder.com/300x200',
    link: '#'
  },
...
const Projects  = ({ setCloud1X, setCloud2X, setCloud3X }: ProjectsProps ) => {
  const { index, direction, setSlide } = useSlideNavigation(projects.length, {
    setCloud1X,
    setCloud2X,
    setCloud3X,
  });
    ...
    <Box display="flex" alignItems="center" justifyContent="center" position="relative" height="80vh" zIndex={4}>
      ...
      <Box width="100%" maxWidth={{ xs: 300, sm: 500, md: 600 }} mx="auto">
        <AnimatePresence custom={direction} initial={false} mode="popLayout">
          <ProjectSlide key={index} project={projects[index]} />
        </AnimatePresence>
      </Box>
...
const ProjectSlide = forwardRef(function ProjectSlide(
  { project }: { project: { title: string; description: string; image: string; link: string } },
    ...
    >
      <Card sx={{
        ...
        <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <CardActions>
            <Button size="small" href={project.link} target="_blank" rel="noopener noreferrer">View</Button>
          </CardActions>
        </CardContent>
      </Card>
    </motion.div>
